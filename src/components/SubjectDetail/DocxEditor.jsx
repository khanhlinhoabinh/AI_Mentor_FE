...
import {
  X, Download, Save, Bold, Italic, Underline, Strikethrough,
  Subscript, Superscript, AlignLeft, AlignCenter, AlignRight,
  AlignJustify, List, ListOrdered, Image as ImageIcon, Table,
  Link, Minus, Quote, Undo2, Redo2, Search, CheckCircle2,
  Circle, Type, ChevronLeft, ChevronRight,
} from "lucide-react";
import {
...
const FSIZES = [8, 9, 10, 11, 12, 13, 14, 16, 18, 20, 22, 24, 28, 32, 36, 48, 72];
...
function ColorBtn({ icon, title, defaultColor, onApply }) {
  ...
  return (
    <div className="de-color-wrap" title={title}
      onClick={() => inputRef.current?.click()}>
      <div className="de-color-preview">
        <span style={{ fontSize: 11, fontWeight: 700, lineHeight: 1 }}>{icon}</span>
...
export default function DocxEditor({ doc, subjectId, onClose, onSaved }) {
  ...
  const splitPageAt = useCallback((pageIdx) => {
    ...
    setTimeout(() => {
      ...
      if (nextEl) {
        const html = overflowHtml + (pageRefs.current[pageIdx + 1]?.innerHTML || "");
  ...
  const handlePageKeyDown = useCallback((e, pageIdx) => {
    // Keyboard shortcuts chung
    if (isCmd(e) && e.key === "s") { e.preventDefault(); handleSave(); }
    if (isCmd(e) && e.key === "f") { e.preventDefault(); setShowFind(v => !v); }
    if (isCmd(e) && e.key === "z") { e.preventDefault(); exec("undo"); }
    if (isCmd(e) && e.key === "y") { e.preventDefault(); exec("redo"); }
  ...
  const handleDownload = async () => {
    ...
    pageRefs.current.filter(Boolean).forEach((pageEl, pi) => {
      ...
      const nodes = pageEl.querySelectorAll("p,h1,h2,h3,h4,h5,h6,blockquote,pre,li");
      ...
      const parseRuns = (el) => {
        ...
        const walk = (node) => {
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            while (cur && cur !== el) {
              ...
              const s = cur.style ?? {};
              if (tag === "b" || tag === "strong" || s.fontWeight === "bold") bold = true;
              if (tag === "i" || tag === "em" || s.fontStyle === "italic") italics = true;
              if (tag === "u" || s.textDecoration?.includes("underline")) underline = true;
              if (tag === "s" || tag === "strike") strike = true;
              if (s.color) {
                const hex = s.color.match(/\d+/g);
                if (hex?.length >= 3)
                  color = hex.slice(0, 3).map(n => parseInt(n).toString(16).padStart(2, "0")).join("");
              }
              cur = cur.parentElement;
            }
            runs.push(new TextRun({
              text, bold, italics, strike, color,
        ...
        return runs.length
          ? runs
          : [new TextRun({ text: el.innerText || "", size: currentSize * 2, font: currentFont })];
      };
      ...
      nodes.forEach(el => {
        ...
        if (tag === "h1") children.push(new Paragraph({ heading: HeadingLevel.HEADING_1, children: runs, alignment: makeAlignment(el) }));
        else if (tag === "h2") children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, children: runs, alignment: makeAlignment(el) }));
        else if (tag === "h3") children.push(new Paragraph({ heading: HeadingLevel.HEADING_3, children: runs, alignment: makeAlignment(el) }));
        else children.push(new Paragraph({ children: runs, alignment: makeAlignment(el), spacing: { after: 80 } }));
      });
    });
  ...
  const renderToolbar = () => {
    if (activeRibbon === "Trang chủ") return (
      <>
        ...
        <div className="de-toolbar-sep" />
        <ColorBtn icon="A" title="Màu chữ" defaultColor="#111827" onApply={c => exec("foreColor", c)} />
        <ColorBtn icon="▌" title="Màu nền chữ" defaultColor="#ffff00" onApply={c => exec("hiliteColor", c)} />
        <div className="de-toolbar-sep" />

        <select className="de-select" style={{ width: 102 }}
          defaultValue=""
        ...
        <div className="de-toolbar-group">
          <button className="de-tool-btn"
            ...
            title="Danh sách bullet"><List size={13} /></button>
          <button className="de-tool-btn"
            onMouseDown={e => { e.preventDefault(); exec("insertOrderedList"); }}
            title="Danh sách số"><ListOrdered size={13} /></button>
          <button className="de-tool-btn de-tool-btn-wide"
            onMouseDown={e => { e.preventDefault(); exec("indent"); }}
            title="Tăng thụt lề">→</button>
          <button className="de-tool-btn de-tool-btn-wide"
            onMouseDown={e => { e.preventDefault(); exec("outdent"); }}
            title="Giảm thụt lề">←</button>
        </div>
        ...
        <div className="de-toolbar-group">
          <button className="de-tool-btn"
            onMouseDown={e => { e.preventDefault(); exec("undo"); }} title="Hoàn tác (Ctrl+Z)">
            <Undo2 size={13} /></button>
          <button className="de-tool-btn"
            onMouseDown={e => { e.preventDefault(); exec("redo"); }} title="Làm lại (Ctrl+Y)">
    ...
    if (activeRibbon === "Chèn") return (
      <>
        <div className="de-table-picker-wrap">
          <button className="de-tool-btn" title="Chèn bảng"
          ...
          {showTablePicker && (
            <div className="de-table-picker">
              ...
              <div className="de-table-hint">{tableHover.r + 1} × {tableHover.c + 1} bảng</div>
            </div>
          )}
        </div>

        <button className="de-tool-btn" title="Chèn ảnh"
          onClick={() => imgInputRef.current?.click()}>
          <ImageIcon size={13} />
        </button>
        <button className="de-tool-btn" title="Chèn liên kết" onClick={insertLink}>
          <Link size={13} />
        </button>
        <button className="de-tool-btn" title="Đường kẻ ngang"
          onMouseDown={e => { e.preventDefault(); exec("insertHorizontalRule"); }}>
          <Minus size={13} />
        </button>
        <button className="de-tool-btn" title="Trích dẫn"
          onMouseDown={e => { e.preventDefault(); exec("formatBlock", "blockquote"); }}>
          <Quote size={13} />
        </button>
        <button className="de-tool-btn de-tool-btn-wide" title="Xóa định dạng"
    ...
    if (activeRibbon === "Bố cục") return (
      <>
        <span style={{ fontSize: 12, color: "#6b7280", fontWeight: 500 }}>Zoom:</span>
        <select className="de-select" style={{ width: 68 }}
          value={zoom}
        ...
        {/* Xóa trang hiện tại */}
        {pages.length > 1 && (
          <button className="de-tool-btn de-tool-btn-wide de-danger"
            onClick={() => {
              if (!window.confirm(`Xóa trang ${currentPage + 1}?`)) return;
              setPages(prev => {
                const next = [...prev];
    ...
    if (activeRibbon === "Xem") return (
      <span style={{ fontSize: 12, color: "#6b7280" }}>
        {wordCount} từ • {charCount} ký tự • {pages.length} trang • Zoom {zoom}%
      </span>
    );
  ...
  /* ── RENDER ── */
  return (
    <div className="de-overlay" onClick={e => e.target === e.currentTarget && handleClose()}>
      <div className="de-modal">
        ...
        <div className="de-titlebar">
          ...
          <div className="de-titlebar-right">
            ...
            <button className="de-title-btn save" onClick={handleSave} disabled={saving}>
              <Save size={13} />
        ...
        {showFind && (
          <div className="de-find-bar">
            ...
            <input className="de-find-input" value={findText}
              onChange={e => setFindText(e.target.value)}
              onKeyDown={e => e.key === "Enter" && handleFind()}
              placeholder="Từ cần tìm..." />
            <button className="de-find-btn" onClick={handleFind}>Tìm & tô màu</button>
            <div className="de-toolbar-sep" />
            <span className="de-find-label">Thay bằng:</span>
            <input className="de-find-input" value={replaceText}
              onChange={e => setReplaceText(e.target.value)}
              placeholder="Từ thay thế..." />
            <button className="de-find-btn primary" onClick={handleReplace}>Thay tất cả</button>
            {replaceCount !== null && (
              <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 600 }}>
                ✓ Đã thay {replaceCount} chỗ
              </span>
            )}
            <button className="de-find-btn" onClick={() => setShowFind(false)}>✕</button>
          </div>
        )}
            ...
            <div className="de-page-nav">
              <button
                ...
                onClick={() => {
                  ...
                  pageRefs.current[prev]?.scrollIntoView({ behavior: "smooth", block: "center" });
                }}>
                <ChevronLeft size={12} />
              </button>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  pageRefs.current[next]?.scrollIntoView({ behavior: "smooth", block: "center" });
                }}>
      ...
      <input ref={imgInputRef} type="file" accept="image/*"
        className="de-hidden" onChange={handleImageFile} />
    </div>
  );
}