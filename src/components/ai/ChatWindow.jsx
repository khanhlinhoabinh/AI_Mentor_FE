...
const ChatWindow = ({ onClose, onMinimize }) => {
  ...
  const handleSelectSession  = (session) => { setActiveSession(session); setShowHistory(false); };
  const handleDeleteSession  = async (id) => {
    try {
      await deleteSession(id);
      setSessions(prev => prev.filter(s => s.conversationId !== id));
      if (activeSession?.conversationId === id) { setActiveSession(null); setMessages([]); }
  ...
  const handleNewSession = async () => {
    ...
    try {
      ...
      const s = { conversationId: data.conversationId, title: "Cuộc hội thoại mới", createdAt: new Date().toISOString() };
      setActiveSession(s);
      setMessages([]);
      setSessions(prev => [s, ...prev]);
    } catch { setError("Không thể tạo cuộc hội thoại mới."); }
  };
  ...
  const handleSend = useCallback(async (text) => {
    ...
    if (!sessionId) {
      if (!userId) { setError("Bạn cần đăng nhập."); return; }
      try {
        const data = await createSession(userId, (content || attachedFile?.name || "Cuộc hội thoại mới").slice(0, 50));
        sessionId = data.conversationId;
        currentSession = { conversationId: sessionId, title: content.slice(0,50), createdAt: new Date().toISOString() };
        setActiveSession(currentSession);
        setSessions(prev => [currentSession, ...prev]);
      } catch { setError("Không thể tạo cuộc hội thoại."); return; }
    }
        ...
        <div className="chat-input-area">
          ...
          <div className="chat-input-box">
            <textarea
              ...
              placeholder={attachedFile ? "Hỏi gì đó về tài liệu..." : "Nhập tin nhắn..."}
              value={inputValue}
